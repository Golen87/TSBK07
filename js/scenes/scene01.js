var scene01 = new Scene(function() {
	...
	for (var x = -k; x < k; x++) for (var y = 0.25; y < 2*k; y++) for (var z = -k; z < k; z++) {
		addModel(sphere_mesh, [3*x, 3*y, 3*z], [0.2, 0.2, 0.2]);
	}

	addModel(sphere_mesh, [3.5, 1, -6]);
	addModel(sphere_mesh, [3.5, 3, -6], [1, 0.5, 1], [0, 0, 0.25 * Math.PI]);
	addModel(sphere_mesh, [3.5, 1, -3])
		.setTexture(textures.grass_lab);
	addModel(sphere_mesh, [3.5, 3, -3.0], [1, 0.5, 1], [0, 0, 0.25 * Math.PI])
		.setTexture(textures.grass_lab);

	addModel(cube_mesh, [-3.5, 1.0, -6.0]);
	addModel(cube_mesh, [-3.5, 1.0, -3.0])
		.setTexture(textures.grass_lab);
